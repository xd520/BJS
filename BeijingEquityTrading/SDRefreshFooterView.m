...
- (void)layoutSubviews
{
    ...
    NSLog(@"%.2f",_originalScrollViewContentHeight);
    
    /*
     float hight = [[UIScreen mainScreen] bounds].size.height - 104 - 49;
     self.center = CGPointMake(self.scrollView.sd_width * 0.5, hight + self.sd_height * 0.5);
     */
    
    
    self.center = CGPointMake(self.scrollView.sd_width * 0.5, self.scrollView.contentSize.height + self.sd_height * 0.5);
    
    NSLog(@"%.2f  %.2f  %.2f",self.center.x,self.center.y,self.sd_height);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    ...
    NSLog(@"%.2f %.2f %.2f",self.scrollView.contentSize.height,_originalScrollViewContentHeight,self.scrollView.sd_height);
